...
class VBPR_model(nn.Module):
    def __init__(self, num_user, num_video, latent_dim, feature_dim, features):
        ...
        self.v_feat_layer = nn.Embedding(num_video, feature_dim)
        self.v_feat_layer.weight = nn.Parameter(torch.tensor(features, requires_grad=False))
        self.tranfer_layer = nn.Linear(feature_dim, latent_dim)
    ...
    def forward(self, batch_user, batch_video):
        ...
        batch_v_feat = nn.functional.relu(self.tranfer_layer(self.v_feat_layer(batch_video)))        
